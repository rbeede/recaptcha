<?php
# Customizations by Rodney Beede, 2010-11-02

# anti-recaptcha v0.2e (c)opyleft 2010 http://wegeneredv.de/arc **************
# ****************************************************************************
# future fixes and changes:
# - gaining greater recognization rates by using wijipedia as wordlist
...
$word1_contrast_ratio = 0;
if($word1_white_pixel != 0) {
	$word1_contrast_ratio=($word1_black_pixel/$word1_white_pixel);
} else {
	$word1_contrast_ratio = $selective_contrast_threshold;  # Patch by Rodney Beede
}
$word2_black_pixel=0;
$word2_white_pixel=0;
for($x=$second_word_start;$x<$second_word_end;$x++){
...
# debug **********************************************************************
#rbeede,2010-11-04, I need the debug image for showing-- if($debug_mode){
	$debug=imagecreatefrombmp('resized.bmp');
	for($x=0;$x<$width;$x++){
		imagesetpixel_rgb($debug,$x,$mid[$x],255,0,0);
	}
	imagejpeg($debug,"debug.jpg");
#rbeede,2010-11-04, I need the debug image for showing--}
# remove distortion **********************************************************
for($x=0;$x<$width;$x++){
	if($bottom[$x]-$top[$x]>0){
...
if($debug_mode){
	...
	#@unlink('cropped.jpg');
	@unlink('contrast.bmp');
	#rbeede, I need this-- @unlink('debug.jpg');
	
	$input_basename = basename ( $input_image );
	$input_basename = substr($input_basename, 0, strlen($input_basename) - 4);
	
	rename ( 'output.txt', "./results/" . $input_basename . ".txt" );
	rename ( 'cropped.jpg', "./results/" . $input_basename . "--cropped.jpg" );
	rename ( 'debug.jpg', "./results/" . $input_basename . "--debug.jpg" );
}
?>
